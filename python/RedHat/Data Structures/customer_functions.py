def get_customers():
    ...
    # use a list comprehension to convert to a nested list of lists (each a list of strings)
    return [customer.rstrip().split(",") for customer in customer_list]


def get_info(customers):
    print("Nested Structure:", type(customers), type(customers[0]), type(customers[0][0]))
...
def user_interaction(customers):
    tags = ["FirstName", "LastName", "Streets", "City", "State", "ZipCode"]
    fmt = "{:16}{:16}{:20}{:16}{:6}{:5}"
    while True:
